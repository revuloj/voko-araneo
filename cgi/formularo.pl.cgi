...
BEGIN
{
  ...
  $postmaster        = '';
  @referers          = qw(familioj.wikispaces.com localhost);
  @allow_mail_to     = qw(ref2018@steloj.de ref@steloj.de wolfram@steloj.de);
  @recipients        = ();
  %recipient_alias   = ();
...
sub request {
 ...
 <body>
  <h1>Eraro en la skripto</h1>
  <p>
   Okazis eraro en la programo!
  </p>
  <p>
   $message
  </p>
...
print ($self->get_user_email);
print ($self->get_user_realname);
#print ("xx".$self->{FormConfig}{email});
#print ($self->param('Retposhto_1'));
#print($self->cgi_object->param('Retposhto_1'));
...
<p>
  La HTML-formularo eraris pri la metodo POST, do nenio
  okazos nun sekve de via send-peto!!!
</p>
<p>
  Turnu vin al la administranto por korekti la problemon.
  &lt;ref(ĉe)steloj.de&gt;
</p>
END
    return 0;
...
sub get_user_email {
  ...
  my $email = $self->{FormConfig}{email} || $self->param('Retposhto_1');
...
sub get_user_realname {
  ...
  my $realname = $self->{FormConfig}{realname} || $self->param('PersonaNomo_1');
  if (defined $realname) {
...
sub send_main_email_header {
  ...
  my $subject = $self->{FormConfig}{subject} || 'Sendo de TTT-formularo';
  if ($self->{CFG}{secure}) {
...
sub success_page_html_preamble {
  ...
  my $title = $self->escape_html( $self->{FormConfig}{'title'} || 'Dankon' );
  my $torecipient = 'al ' . $self->escape_html($self->{FormConfig}{'recipient'});
...
END
  ...
  <body $attr>
    ...
    <p>Jen kion vi sendis $torecipient je $date</p>
