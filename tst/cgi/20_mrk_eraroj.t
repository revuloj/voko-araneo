...
# 1. parametroj
my $url = "http://0.0.0.0:8088/cgi-bin/mrk_eraroj.pl";
...
if ($content) {
    ...
    if (!$@ && $hashref) {
        ...
        ok(exists $hashref->{drv}, "Respondo enhavas 'drv'");
        ok(exists $hashref->{snc}, "Respondo enhavas 'snc'");
        ok(exists $hashref->{hom}, "Respondo enhavas 'hom'");

        # FARENDA:
        # por testi, ĉu trovi erarojn entute funkcias, ni devus
        # intence enmeti specifajnerarojn en la datumbazon unue
        # ekz-e per admin/art_db.pl
